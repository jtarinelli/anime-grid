...
const Grid: FC<GridProps> = ({ correctGuesses, clues, isAlreadyGuessed, addGuess, isGameOver }) => {
    ...
    for (let row = 0; row < cellsPerSide; row++) {
        for (let col = 0; col < cellsPerSide; col++) {
            ...
            if (col === 0 && row === 0) {
                ...
                cells.push(<div key={key} className="text-sm">{getClueString(clues[horizontalClueIndex])}</div>);
            } else if (col === 0) {
                cells.push(<div key={key} className="text-sm">{getClueString(clues[verticalClueIndex])}</div>);
            } else {
                cells.push(
                    <Cell
    ...
    const threeByThreeGrid = `w-full md:h-full md:w-auto aspect-[230/300] grid grid-cols-[5rem_repeat(2,_1fr)] grid-rows-[3rem_repeat(2,_1fr)] gap-2 p-6 place-items-center text-center`;
    const fourByFourGrid = `w-full md:h-full md:w-auto aspect-[230/300] grid grid-cols-[5rem_repeat(3,_1fr)] grid-rows-[3rem_repeat(3,_1fr)] gap-2 p-6 place-items-center text-center`;
